...
const Deletemodal = ({
  ...
  const handleClose = () => {
    ...
    if (currentthough) {
      dispatch(thoughDeleteAction({ thoughtId: currentthough },()=>{
        if(location.state === "challenges" || location.state === "Challenges"){
      ...
      dispatch(actionItemDeleteAction({ actionId: deleteActionId },()=>{viewActionADta()}),
      );
    }
    setShow(false);
  }
          ...
          <div className="detelemodal-box">
            ...
            <div className="quescbtn-box dismiss-modal">
              <Button
                text="Yes"
                addedClass="yes"
                onClick={handleClose || deleteFact}
              />
